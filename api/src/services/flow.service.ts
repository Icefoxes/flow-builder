...
class FlowService {
      ...
      .aggregate([
        ...
        { $match: { "nodes.data.label": { $regex: text } } },
        {
          $project: {
            ...
            flowName: "$name",
            nodeId: "$nodes.id",
            nodeName: "$nodes.data.label",
            nodeType: "$nodes.data.nodeType",
            alias: "$alias",
            _id: 0,
          },
        },
  ...
  public async createFlow(flow: Flow): Promise<Flow> {
    ...
    if (findFlow)
      throw new HttpException(
        400,
        `flow alias ${findFlow.alias} already exist`
      );

    const findTeam = await teamModel.findById(new ObjectId(flow.teamId)).exec();
    if (!!!findTeam)
      throw new HttpException(400, `flow team id ${flow.teamId} not exist`);
  ...
  public async UpdateFlow(flowId: string, flow: Flow): Promise<Flow> {
    const findFlow = await flowModel.findOne({ alias: flow.alias }).exec();
    if (findFlow && findFlow.id != new ObjectId(flowId))
      throw new HttpException(409, `flow alias ${findFlow.name} already exist`);

    const findTeam = await teamModel.findById(new ObjectId(flow.teamId)).exec();
    if (!!!findTeam)
      throw new HttpException(400, `flow team id ${flow.teamId} not exist`);

    const updatedFlow = await flowModel
      .findByIdAndUpdate(new ObjectId(flowId), flow, { new: true })
      .exec();
  ...
  public async DeleteFlow(flowId: string): Promise<string> {
    const flowTobeDeleted = await flowModel
      .findByIdAndDelete(new ObjectId(flowId))
      .exec();
