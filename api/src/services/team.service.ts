...
class TeamService {
  ...
  public async createTeam(team: Team): Promise<Team> {
    ...
    if (findTeam)
      throw new HttpException(400, `team name ${findTeam.name} already exist`);
  ...
  public async UpdateTeam(teamId: string, team: Team): Promise<Team> {
    const findTeam = await teamModel.findOne({ name: team.name }).exec();
    if (findTeam && findTeam.id != new ObjectId(teamId))
      throw new HttpException(409, `team name ${findTeam.name} already exist`);

    const updatedTeam = await teamModel
      .findByIdAndUpdate(new ObjectId(teamId), team, { new: true })
      .exec();
  ...
  public async DeleteTeam(teamId: string): Promise<string> {
    const teamTobeDeleted = await teamModel
      .findByIdAndDelete(new ObjectId(teamId))
      .exec();
